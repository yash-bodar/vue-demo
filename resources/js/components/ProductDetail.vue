  ...
  <div class="container-xl py-4">
    ...
    <div v-else-if="product" class="card card-vuexy p-4 mb-4">
      <div class="row g-4">
        ...
        <div class="col-md-5">
          <div class="product-card-img-wrapper rounded-3 position-relative shadow-sm" style="aspect-ratio: 1/1;">
        ...
        <div class="col-md-7 d-flex flex-column">
          <div class="mb-2">
            <span class="badge bg-label-primary fs-8 me-2">{{ product.category?.name || 'Category' }}</span>
          ...
          <!-- Description -->
          <p class="text-muted mb-4 fs-7" style="line-height: 1.7;">
          ...
          <div class="row g-3 mb-4">
            <div class="col-6">
              <div class="p-3 rounded-3 bg-light border-subtle border">
                ...
                <span class="fw-bold fs-6" :class="currentStock > 0 ? 'text-success' : 'text-danger'">
          ...
          <div class="mt-auto pt-3 border-top">
            <template v-if="user?.role === 'user'">
              <div v-if="getCartQuantity(product.id) > 0" class="d-inline-flex align-items-center gap-3 p-2 bg-light rounded-pill border">
                <button type="button" class="btn btn-sm btn-primary rounded-circle btn-icon" :disabled="loadingProductId == product.id" @click="updateCart(product.id, 'decrease')">
                  <i class="fas fa-minus fs-8"></i>
                </button>
                <span class="fw-bold px-2 fs-6">{{ getCartQuantity(product.id) }}</span>
                <button type="button" class="btn btn-sm btn-primary rounded-circle btn-icon" :disabled="getCartQuantity(product.id) >= currentStock || loadingProductId === product.id" @click="updateCart(product.id, 'increase')">
                  <i class="fas fa-plus fs-8"></i>
                </button>
              </div>

              <div v-else-if="currentStock > 0" class="d-flex gap-3">
                <button class="btn btn-primary rounded-pill px-4 py-2.5 shadow-primary fw-bold" @click="updateCart(product.id, 'increase')">
                  <i class="fas fa-cart-shopping me-2"></i>Add to Cart
                </button>
                <router-link to="/checkout" class="btn btn-outline-primary rounded-pill px-4 py-2.5 fw-bold" @click="updateCart(product.id, 'increase')">
                  Buy Now
                </router-link>
              </div>
            ...
            <template v-else-if="user?.role === 'admin'">
              <button class="btn btn-primary rounded-pill px-4 py-2.5 shadow-primary fw-bold" @click="$router.push(`/products/edit/${product.id}`)">
    ...
    <div v-if="product" class="card card-vuexy p-4">
      ...
      <div v-if="user?.role === 'user'" class="p-4 rounded-3 bg-light border-subtle border mb-4">
        ...
        <div class="mb-3">
          ...
          <div class="d-flex gap-2">
            <template v-for="i in 5" :key="i">
              <i class="fa-star fs-4 cursor-pointer text-warning" :class="i <= newRating.rating ? 'fas' : 'far'" @click="newRating.rating = i"></i>
            </template>
          </div>
        </div>
        <div class="mb-3">
          <label class="form-label fs-7 fw-semibold">Your Feedback</label>
          <textarea class="form-control" v-model="newRating.review" rows="3" placeholder="Share your experience with this product..."></textarea>
        </div>
        <button class="btn btn-primary rounded-pill px-4 fw-bold" @click="submitReview" :disabled="submitting">
      ...
      <div v-else-if="ratings.length > 0" class="d-flex flex-column gap-3">
        <div v-for="rating in ratings" :key="rating.id" class="p-3 rounded-3 bg-light border-subtle border">
...
export default {
  ...
  computed: {
    ...
    currentPrice() {
      ...
      return this.product?.converted_price || 0
    },
    currentStock() {
  ...
  methods: {
    ...
    async fetchProduct() {
      ...
      try {
        ...
        const response = await this.$axios.get(`/api/get-product-details/${id}`)
    ...
    async fetchRatings() {
      ...
      try {
        const id = this.$route.params.id
        const response = await this.$axios.get(`/api/get-ratings/${id}`)
        if (response.data.success) {
    ...
    async updateWishlist(productId, action) {
      ...
      try {
        ...
        if (response.data.success) {
          this.product.wishlist = action === 'add'
        }
      } catch (err) {
    ...
    async submitReview() {
      ...
      try {
        const response = await this.$axios.post('/api/add-rating', {
