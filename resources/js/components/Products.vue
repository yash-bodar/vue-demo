<template>
  <div class="card card-vuexy card-admin-spaced p-4">
    <!-- Header Controls -->
    <div
      ...
      <table class="table table-hover align-middle mb-0">
        <thead>
          <tr>
            ...
            <th @click="sortByField('category_id', 'fetchProducts')" class="cursor-pointer">
              Category
            </th>
            <th @click="sortByField('status', 'fetchProducts')" class="cursor-pointer">Status</th>
            <th @click="sortByField('created_at', 'fetchProducts')" class="cursor-pointer">
              Created
            </th>
        ...
        <tbody>
          <tr v-if="productsList.length > 0" v-for="product in productsList" :key="product.id">
            <td class="cursor-pointer" @click="$router.push(`/product/detail/${product.id}`)">
              <div class="d-flex align-items-center gap-3">
                <img
                  class="rounded-3 border product-thumb-sq"
                  width="44"
                  height="44"
            ...
            <td>
              <span class="badge bg-label-info fs-8">{{
    ...
    >
      <small class="text-muted fs-8"
        ...
        {{ Math.min(currentPage * perPage, productCount) }} of {{ productCount }} entries</small
      >
      <nav>
