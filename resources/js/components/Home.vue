    ...
    <div class="hero-banner mb-5 text-white position-relative overflow-hidden shadow-vuexy-lg">
      <div class="row align-items-center position-relative">
        <div class="col-lg-7 px-lg-4">
          ...
          <h1 class="display-4 fw-bold mb-3 text-heading text-white">
            Discover Luxury & Modern Shopping
          </h1>
          <p class="fs-5 opacity-90 mb-4 text-max-580">
            Explore handpicked fashion collections, electronics, and accessories. Enjoy instant
            checkout with Stripe and express global shipping.
          </p>
          <div class="d-flex flex-wrap gap-3">
            <router-link
              to="/product"
              class="btn btn-primary btn-lg px-4 py-3 rounded-pill fw-bold shadow-primary"
            >
              <i class="fas fa-bag-shopping me-2"></i>Shop Collection
            </router-link>
            <router-link
              to="/product"
              class="btn btn-outline-light btn-lg px-4 py-3 rounded-pill fw-bold"
            >
        ...
        <div class="col-lg-5 text-center d-none d-lg-block">
          <div
            class="p-4 bg-white bg-opacity-10 rounded-4 border border-white border-opacity-20 backdrop-blur hero-float-box"
          >
    ...
    <div class="mb-5" v-if="categories.length > 0">
      ...
      <div class="row g-3">
        <div v-for="cat in categories.slice(0, 6)" :key="cat.id" class="col-lg-2 col-md-4 col-6">
          <router-link
            :to="{ path: '/product', query: { category: cat.id } }"
            class="card card-vuexy p-3 text-center text-decoration-none h-100 cursor-pointer"
          >
            <div class="badge bg-label-primary rounded-circle p-3 mx-auto mb-2 badge-icon-56">
    ...
    <div class="row g-4 mb-5">
      <div class="col-md-6 col-lg-3">
        <div class="card card-vuexy p-4 text-center h-100">
          <div class="badge bg-label-primary rounded-circle p-3 mx-auto mb-3 badge-icon-56">
            <i class="fas fa-truck-fast fs-4"></i>
          </div>
          <h5 class="fw-bold mb-2">Free Express Delivery</h5>
          <p class="text-muted fs-7 mb-0">
            Free priority shipping on all qualified orders over $99 USD.
          </p>
        </div>
      </div>
      ...
      <div class="col-md-6 col-lg-3">
        <div class="card card-vuexy p-4 text-center h-100">
          <div class="badge bg-label-success rounded-circle p-3 mx-auto mb-3 badge-icon-56">
      ...
      <div class="col-md-6 col-lg-3">
        <div class="card card-vuexy p-4 text-center h-100">
          <div class="badge bg-label-warning rounded-circle p-3 mx-auto mb-3 badge-icon-56">
            <i class="fas fa-rotate-left fs-4"></i>
          </div>
          <h5 class="fw-bold mb-2">30-Day Money Back</h5>
          <p class="text-muted fs-7 mb-0">
            Hassle-free 30 days return policy on eligible catalog goods.
          </p>
        </div>
      </div>
      ...
      <div class="col-md-6 col-lg-3">
        <div class="card card-vuexy p-4 text-center h-100">
          <div class="badge bg-label-info rounded-circle p-3 mx-auto mb-3 badge-icon-56">
        ...
        <div v-for="product in featuredProducts" :key="product.id" class="col-lg-3 col-md-6">
          <div class="card card-vuexy h-100">
            ...
            <div class="p-3 d-flex flex-column flex-grow-1">
              <small class="text-muted fs-8 mb-1 text-uppercase">{{
                product.category_name || 'Store Item'
              }}</small>
              <router-link
                :to="`/product/detail/${product.id}`"
                class="text-heading fw-bold fs-6 text-decoration-none mb-2 text-truncate"
              >
                {{ product.name }}
              </router-link>

              <div
                class="mt-auto d-flex align-items-center justify-content-between pt-2 border-top"
              >
                <div class="fw-bold text-primary fs-5">
                  {{ formatCurrency(product.price || product.converted_price) }}
                </div>
                <button
  ...
  methods: {
    ...
    async fetchHomeData() {
      ...
      try {
        const [catRes, prodRes] = await Promise.all([
          ...
          this.$axios.get('/api/get-products'),
        ])
        this.categories =
          catRes.data.categories || catRes.data.data?.data || catRes.data.data || catRes.data || []
        const prods =
    ...
    async addToCart(prod) {
      ...
      if (cartStore.addToCart) {
        ...
        await this.$axios.post('/api/update-cart', { product_id: prod.id, action: 'increase' })
      }
    },
    async toggleWishlist(prod) {
      ...
      if (wishlistStore.toggleWishlist) {
        ...
        await this.$axios.post('/api/update-wishlist', { product_id: prod.id, action: 'add' })
      }
    },
  },
}
