<template>
  <div class="container-xl py-4">
    ...
    <div v-else-if="cartProducts.length === 0" class="card card-vuexy p-5 text-center">
      ...
      <p class="text-muted fs-7 mb-4">Discover our catalog items and add your favorite goods to cart.</p>
      ...
      <div class="col-lg-8">
        <div class="card card-vuexy p-4 mb-4">
          ...
          <div class="d-flex flex-column gap-3">
            <div v-for="(cartProduct, index) in cartProducts" :key="index" class="p-3 rounded-3 bg-light border-subtle border">
              <div class="row align-items-center g-3">
                <!-- Thumbnail -->
                <div class="col-3 col-sm-2">
                  <img :src="getImageUrl(cartProduct.product.image)" class="rounded-3 img-fluid border" style="aspect-ratio: 1/1; object-fit: cover;" :alt="cartProduct.product.name" />
                </div>
                ...
                <div class="col-9 col-sm-10">
                  <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                      <h6 class="fw-bold text-heading cursor-pointer mb-1" @click="$router.push(`/product/detail/${cartProduct.product.id}`)">
                    ...
                    <div class="d-flex align-items-center gap-4">
                      <div class="d-flex align-items-center border rounded-pill p-1 bg-card">
                        ...
                        <span class="fw-bold px-3 fs-7">{{ getCartQuantity(cartProduct.product.id, cartProduct.product_variant_id) }}</span>
                        <button 
                          class="btn btn-sm btn-icon border-0 text-primary" 
        ...
        <div class="card card-vuexy p-4">
          <div class="d-flex align-items-center justify-content-between mb-3">
            <h5 class="fw-bold text-heading m-0"><i class="fas fa-location-dot me-2 text-primary"></i>Delivery Address</h5>
            <button type="button" @click="addAddress" class="btn btn-sm btn-outline-primary rounded-pill">
          ...
          <div v-else class="d-flex flex-column gap-2">
            <div 
              ...
              :class="selectedAddressId === address.id ? 'border-primary bg-label-primary' : 'bg-light border-subtle'"
              @click="selectAddress(address)"
            >
              <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-3">
                  <input class="form-check-input" type="radio" :checked="selectedAddressId === address.id" @change="selectAddress(address)" />
                  <div>
                    <h6 class="fw-bold m-0 fs-7">{{ address.title || 'Shipping Address' }}</h6>
                    <p class="text-muted fs-8 mb-0">{{ address.address_line_1 }}, {{ address.city }}, {{ address.state }} {{ address.postal_code }}</p>
                  </div>
                </div>
        ...
        <div class="card card-vuexy p-4 mb-4">
          ...
          <div class="d-flex flex-column gap-2 border-bottom pb-3 mb-3 fs-7">
            <div class="d-flex justify-content-between">
              ...
              <span class="fw-bold text-heading">{{ user?.currency_sign }}{{ subtotal.toFixed(2) }}</span>
            </div>
            <div class="d-flex justify-content-between" v-if="appliedCoupon">
              <span class="text-muted">Coupon Discount</span>
              <span class="fw-bold text-success">-{{ user?.currency_sign }}{{ discountAmount.toFixed(2) }}</span>
            </div>
            <div class="d-flex justify-content-between">
          ...
          <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <span class="fw-bold text-primary fs-4">{{ user?.currency_sign }}{{ total.toFixed(2) }}</span>
          </div>

          <!-- Coupon Input -->
          <div class="mb-4">
            <div class="input-group input-group-sm">
              <input type="text" class="form-control" v-model="couponCode" placeholder="Promo / Coupon Code" :disabled="appliedCoupon" />
              <button v-if="!appliedCoupon" class="btn btn-primary" type="button" @click="applyCoupon">Apply</button>
              <button v-else class="btn btn-danger" type="button" @click="removeCoupon">Remove</button>
            </div>
            <small v-if="couponMessage" :class="couponSuccess ? 'text-success' : 'text-danger'" class="d-block mt-1 fs-9 fw-semibold">
              {{ couponMessage }}
            </small>
          </div>

          <!-- Checkout Action Button -->
          <button class="btn btn-primary w-100 rounded-pill py-2.5 fw-bold shadow-primary" @click="checkoutPage">
...
export default {
  ...
  computed: {
    ...
    subtotal() {
      return this.cartProducts.reduce((sum, item) => {
        const price = item.variant ? item.variant.converted_price : item.product.converted_price
    ...
    discountAmount() {
      ...
      return parseFloat(this.appliedCoupon.value || 0)
    },
    total() {
  ...
  methods: {
    ...
    getCartQuantity(productId, variantId) {
      const item = this.cartProducts.find(c => c.product_id === productId && c.product_variant_id === variantId)
    ...
    async applyCoupon() {
      ...
      try {
        const response = await this.$axios.post('/api/apply-coupon', { code: this.couponCode })
        if (response.data.success) {
    ...
    checkoutPage() {
      this.$router.push({ path: '/checkout', query: { address_id: this.selectedAddressId } })
    }
  }
}
</script>