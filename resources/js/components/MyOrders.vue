  ...
  <div class="container-xl py-4">
    ...
    <div class="orders-body">
      ...
      <div v-else-if="!orders || (orders.length === 0)" class="card card-vuexy p-5 text-center">
        ...
        <p class="text-muted fs-7 mb-4">When you purchase items from our store, your order details will appear here.</p>
        <button class="btn btn-primary rounded-pill px-4 py-2 mx-auto" @click="$router.push('/product')">
            ...
            <div class="accordion-item border-0">
                ...
                >
                  <div class="row align-items-center g-3 w-100 m-0">
                    <div class="col-6 col-md-3 p-0">
                      <small class="text-muted fs-8 text-uppercase fw-bold d-block">Order Reference</small>
                      <span class="fw-bold text-primary fs-6">#{{ order.id }}</span>
                    </div>

                    <div class="col-6 col-md-3 p-0">
                      <small class="text-muted fs-8 text-uppercase fw-bold d-block">Date & Items</small>
                      <div class="fs-7 fw-semibold"><i class="far fa-calendar me-1"></i>{{ formatDate(order.created_at) }}</div>
                      <small class="text-muted fs-8">{{ order.order_items?.length || 0 }} Item(s)</small>
                    </div>
                    ...
                    <div class="col-6 col-md-3 p-0 text-md-end text-start">
                      <small class="text-muted fs-8 text-uppercase fw-bold d-block">Total Amount</small>
              ...
              <div :id="'collapse' + order.id" class="accordion-collapse collapse" data-bs-parent="#ordersAccordion">
                <div class="card-body p-4 bg-light border-top">
                  <div v-for="item in order.order_items" :key="item.id" class="p-3 mb-2 rounded-3 bg-card border border-subtle">
                    ...
                    <div class="col-md-5 col-lg-4">
                      <div class="p-3 rounded-3 bg-card border border-subtle fs-7">
                        <div class="d-flex justify-content-between mb-1">
                          ...
                          <span class="fw-bold">{{ user?.currency_sign || '$' }}{{ order.total_amount }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-1">
                          <span class="text-muted">Shipping</span>
                          <span class="fw-bold">{{ user?.currency_sign || '$' }}{{ order.shipping }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                          <span class="text-muted">Discount</span>
                          <span class="fw-bold text-danger">-{{ user?.currency_sign || '$' }}{{ order.discount_amount }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center pt-2 border-top">
                          <span class="fw-bold text-heading">Grand Total</span>
                          <span class="fw-bold text-primary fs-5">{{ user?.currency_sign || '$' }}{{ order.final_amount }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <nav>
              <ul class="pagination mb-0 gap-2">
                <li class="page-item" :class="{ disabled: currentPage === 1 }">
                  <button class="page-link rounded-circle border-0 text-primary" :disabled="currentPage === 1" @click="loadOrders(currentPage - 1)">
                    <i class="fas fa-chevron-left"></i>
                  </button>
                </li>
                <li v-for="page in lastPage" :key="page" class="page-item" :class="{ active: page === currentPage }">
                  <button class="page-link rounded-circle border-0 fw-bold" :class="page === currentPage ? 'bg-primary text-white' : 'text-primary'" @click="loadOrders(page)">
                    {{ page }}
                  </button>
                </li>
                <li class="page-item" :class="{ disabled: currentPage === lastPage }">
                  <button class="page-link rounded-circle border-0 text-primary" :disabled="currentPage === lastPage" @click="loadOrders(currentPage + 1)">
...
export default {
  ...
  methods: {
    ...
    async loadOrders(page = 1) {
      this.loading = true
      try {
        const response = await this.$axios.get(`/api/user-orders?page=${page}`)
